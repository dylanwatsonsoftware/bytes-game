...
export class GameOver extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(cx, 40, 'GAME OVER', {
            fontFamily: 'Courier', fontSize: '48px', fill: '#ff4444', fontStyle: 'bold',
            stroke: '#000', strokeThickness: 4, resolution: this.dpr
        }).setOrigin(0.5).setDepth(1);

        // Score display
        this.add.text(cx, 90, `Score: ${this.finalScore}`, {
            fontFamily: 'Courier', fontSize: '24px', fill: '#ffffff',
            stroke: '#000', strokeThickness: 2, resolution: this.dpr
        }).setOrigin(0.5).setDepth(1);

        // Country flag display
        this.flagText = this.add.text(cx + 80, 128, this.playerFlag, {
            fontSize: '26px', resolution: this.dpr
        }).setOrigin(0.5).setDepth(1);

        // "Enter your name" label
        this.add.text(cx, 128, 'Enter your name:', {
            fontFamily: 'Courier', fontSize: '20px', fill: '#aaddff',
    ...
    createRecentNameChips(cx, y) {
        ...
        this.add.text(cx, y, 'or pick a name:', {
            fontFamily: 'Courier', fontSize: '15px', fill: '#668899',
            resolution: this.dpr
        }).setOrigin(0.5).setDepth(1);
        ...
        names.forEach((name, i) => {
            ...
            const chip = this.add.text(chipX, y + 24, name, {
                fontFamily: 'Courier', fontSize: '16px', fill: '#44aadd', backgroundColor: '#1a2a3a',
                padding: { x: 8, y: 4 },
                resolution: this.dpr
    ...
    createButton(x, y, label, callback) {
        const btn = this.add.text(x, y, label, {
            fontFamily: 'Courier', fontSize: '24px', fill: '#ffffff', backgroundColor: '#226644',
    ...
    showLeaderboard(scores, playerName) {
        ...
        this.add.text(cx, 30, 'LEADERBOARD', {
            fontFamily: 'Courier', fontSize: '40px', fill: '#ffdd44', fontStyle: 'bold',
            stroke: '#000', strokeThickness: 3, resolution: dpr
        }).setOrigin(0.5).setDepth(1);

        // Your score reminder
        this.add.text(cx, 68, `Your score: ${this.finalScore}`, {
            fontFamily: 'Courier', fontSize: '20px', fill: '#88bbdd',
        ...
        scores.forEach((entry, i) => {
            ...
            const rowStyle = (color) => ({
                fontFamily: 'Courier', fontSize: '19px', fill: color,
        ...
        if (scores.length === 0) {
            this.add.text(cx, tableY + 60, 'No scores yet - you could be first!', {
                fontFamily: 'Courier', fontSize: '18px', fill: '#668899', resolution: dpr
            }).setOrigin(0.5).setDepth(1);
        }
